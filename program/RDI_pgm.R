...
out <- CoxWeights(marker=eta1, Stime=ads6$EFS_1, status=ads6$EFS_FLG, predict.time=predict_time)
auc[1] <- out$AUC
...
out <- CoxWeights(marker=eta2, Stime=ads6$OS_DAY, status=ads6$DETH_FLG, predict.time=predict_time)
auc[2] <- out$AUC
...
out <- CoxWeights(marker=eta3, Stime=ads6$OS_DAY, status=ads6$DETH_FLG, predict.time=predict_time)
auc[3] <- out$AUC
...
out <- CoxWeights(marker=eta4, Stime=ads6$OS_DAY, status=ads6$DETH_FLG, predict.time=predict_time)
auc[4] <- out$AUC
...
out <- CoxWeights(marker=eta5, Stime=ads6$OS_DAY, status=ads6$DETH_FLG, predict.time=predict_time)
auc[5] <- out$AUC
...
out <- CoxWeights(marker=eta6, Stime=ads7$EFS_1, status=ads7$EFS_FLG, predict.time=predict_time)
auc[6] <- out$AUC
...
nobs <- length(ads6$EFS_1[ads6$EFS_FLG == 1])
span <- 1.0 * (nobs ^ 0.2)
...
nobs <- length(ads6$OS_DAY[ads6$DETH_FLG == 1])
span <- 1.0 * (nobs ^ 0.2)
